...
export default async function GamePage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="w-full px-8 md:px-12 lg:px-16 xl:px-20">
          <main className="flex-1 max-w-6xl mx-auto my-0 md:my-0 bg-transparent rounded-3xl shadow-none p-6 md:p-8 lg:p-12 xl:p-16 border-none min-h-[70vh] flex flex-col relative items-center">
            ...
            <div className="flex flex-wrap gap-6 items-center justify-center mb-10 mt-6 w-full animate-fade-in-up">
              ...
              {/* Genre */}
              {game.genre && (
                <div className="bg-gradient-to-r from-purple-500 to-purple-600 text-white font-bold px-6 py-3 rounded-2xl shadow-lg border border-purple-400 transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
                  {game.genre}
                </div>
              )}
              
              {/* Platforms */}
              {game.platform && game.platform.map((platform) => (
                <div key={platform} className="bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold px-6 py-3 rounded-2xl shadow-lg border border-blue-400 transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
                  {platform}
                </div>
              ))}
              ...
              {game.technologies && game.technologies.map((tech) => (
                <div key={tech} className="bg-gradient-to-r from-gray-600 to-gray-700 text-white font-bold px-6 py-3 rounded-2xl shadow-lg border border-gray-500 transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
                  {tech}
                </div>
              ))}
            </div>
            ...
            <div className="w-full max-w-4xl mx-auto mb-12">
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 text-center">About the Game</h2>
              <p className="text-lg md:text-xl text-gray-300 leading-relaxed text-center" style={{ lineHeight: 1.8 }}>
            ...
            <div className="grid md:grid-cols-2 gap-8 w-full max-w-4xl mb-12">
              <div className="bg-gray-900 bg-opacity-50 p-6 rounded-2xl border border-gray-700">
                <h3 className="text-xl font-bold text-white mb-4">Game Details</h3>
                <div className="space-y-3 text-gray-300">
                  {game.genre && <div><strong className="text-primary">Genre:</strong> {game.genre}</div>}
                  {game.platform && <div><strong className="text-primary">Platforms:</strong> {game.platform.join(', ')}</div>}
                  {game.releaseDate && <div><strong className="text-primary">Release Date:</strong> {game.releaseDate}</div>}
                </div>
              </div>
              ...
              <div className="bg-gray-900 bg-opacity-50 p-6 rounded-2xl border border-gray-700">
                ...
                <div className="flex flex-wrap gap-3">
                  {game.technologies.map((tech) => (
                    <div key={tech} className="bg-primary bg-opacity-20 text-primary-light px-4 py-2 rounded-xl border border-primary-light">
                      {tech}
                    </div>
                  ))}
                </div>
      ...
      <style jsx global>{`
        .animate-fade-in-up {
          animation: fadeInUp 0.8s cubic-bezier(.4,1.2,.6,1) both;
        }
        .animate-fade-in-up-slow {
          animation: fadeInUp 1.4s cubic-bezier(.4,1.2,.6,1) both;
        }
        @keyframes fadeInUp {
