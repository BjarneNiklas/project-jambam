...
class RateRequest(BaseModel):
    ...
    user_id: str = Field(..., description="The ID (Supabase UUID) of the user giving the rating.")
...
class BuyRequest(BaseModel):
    ...
    buyer_id: str = Field(..., description="The ID (Supabase UUID) of the user buying the asset.")
...
class LicensePurchaseRequest(BaseModel):
    ...
    buyer_id: str = Field(..., description="ID (Supabase UUID) of the user buying the license")
    # In a real app, we'd validate the user has enough credits/currency
...
class UserRequest(BaseModel):
    ...
    # This model might be less relevant if user creation is primarily handled by Supabase auth.
    # Profile creation might be separate.
    username: str = Field(..., description="Username")
    email: str = Field(..., description="Email address")
    # Supabase ID will be generated by Supabase, not passed in request here.
    avatar_url: Optional[str] = Field(None, description="Avatar URL for profile")
    bio: Optional[str] = Field(None, description="User bio for profile")

class UserUpdateRequest(BaseModel):
    """Request model for updating a user's profile."""
    # User ID will typically come from the path or token, not the body for an update.
    username: Optional[str] = Field(None, description="Username")
    avatar_url: Optional[str] = Field(None, description="Avatar URL")
    bio: Optional[str] = Field(None, description="User bio")

class UserResponse(BaseModel): # Represents a user profile
    """Response model for user details (profile)."""
    id: str # Supabase User ID (UUID)
    username: str
    email: str # Usually from Supabase auth user, can be in profile too
    avatar_url: Optional[str]
    bio: Optional[str]
    created_at: Optional[datetime] # Profile creation time
    updated_at: Optional[datetime] # Profile update time
...
class CommunityThemeRequest(BaseModel):
    ...
    created_by: str = Field(..., description="User ID (Supabase UUID) who created the theme")
